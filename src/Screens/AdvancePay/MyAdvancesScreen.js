...
import { ADVANCE_PAY_NAVIGATION } from '../../Navigation/route_names';
        ...
        <View style={styles.amountRow}>
          ...
          {isAmountEdited && (
            <>
              ...
              <View style={styles.amountItem}>
                ...
                <Text style={[styles.amountValue, { color: '#34D399' }]}>
          ...
          {item.status === 'Approved' && (
            <View style={[styles.payBadge, { backgroundColor: payConfig.bg }]}>
              <Text style={[styles.payBadgeText, { color: payConfig.color }]}>
                {item.paymentStatus}
              </Text>
            </View>
          )}
          ...
          <View style={styles.progressBar}>
            <View
              style={[
                ...
                {
                  width: `${Math.min(
                    (paidAmount / displayAmount) * 100,
                    100,
                  )}%`,
                  backgroundColor:
                    paidAmount >= displayAmount ? '#34D399' : '#F59E0B',
                },
              ]}
            />
          </View>
            ...
            {item.lastPaymentMethod ? (
              <View style={styles.detailRow}>
                ...
                <Text style={styles.detailValue}>
                  ₹{formatAmount(item.lastPaymentAmount)} via{' '}
                  {item.lastPaymentMethod}
                </Text>
              </View>
            ) : null}
  ...
  useEffect(() => {
    ...
    const unsubscribe = firestore()
      ...
      .onSnapshot(
        ...
        error => {
          console.log('Error listening to advances:', error);
          setLoading(false);
          setRefreshing(false);
        },
      );
    ...
    <View>
      ...
      <View style={styles.pillRow}>
        {Object.entries(counts).map(([status, count]) => (
          <View
            key={status}
            style={[styles.pill, { backgroundColor: STATUS_CONFIG[status].bg }]}
          >
            <Text
              style={[styles.pillText, { color: STATUS_CONFIG[status].color }]}
            >
              {count} {status}
            </Text>
          </View>
      ...
      >
        ...
        <View>
          ...
          <Text style={styles.headerSubtitle}>
            {advances.length} total request{advances.length !== 1 ? 's' : ''}
          </Text>
        </View>
          ...
          <TouchableOpacity
            ...
            onPress={() =>
              props.navigation.navigate(
                ADVANCE_PAY_NAVIGATION.proceed_to_req_advance_pay,
              )
            }
          >
          ...
          ListEmptyComponent={
            <View style={styles.emptyContainer}>
              ...
              <Text style={styles.emptySubtitle}>
                Tap + to request an advance payment
              </Text>
              <TouchableOpacity
