...
const ActionModal = ({ visible, advance, onClose, onConfirm }) => {
  ...
  const handleConfirm = () => {
    if (!action) { Alert.alert('Select Action', 'Please choose to Approve or Reject.'); return; }
    if (action === 'approve') {
      if (!parseFloat(editedAmount) || parseFloat(editedAmount) <= 0) {
        Alert.alert('Invalid Amount', 'Please enter a valid approved amount.'); return;
      }
      if (!paymentMethod) {
        Alert.alert('Select Payment Method', 'Please select how you paid the advance.'); return;
      }
    }
        ...
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ width: '100%' }}>
          <View style={styles.modalSheet}>
            ...
            <View style={styles.modalEmployeeRow}>
              ...
              <View style={{ flex: 1 }}>
                <Text style={styles.modalEmployeeName}>{advance.employeeName}</Text>
                <Text style={styles.modalEmployeeEmail}>{advance.employeeEmail}</Text>
              </View>
            </View>
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
              ...
              <View style={styles.requestedBanner}>
                ...
                <Text style={styles.requestedAmount}>
                  ₹{parseFloat(advance.requestedAmount || 0).toLocaleString('en-IN', { maximumFractionDigits: 2 })}
                </Text>
              ...
              <View style={styles.actionRow}>
                <TouchableOpacity
                  style={[styles.actionBtn, action === 'approve' && styles.actionBtnApprove]}
                  onPress={() => setAction('approve')}
                >
                  <Icon name="check-circle-outline" size={22} color={action === 'approve' ? '#34D399' : '#9CA3AF'} />
                  <Text style={[styles.actionBtnText, action === 'approve' && { color: '#34D399', fontWeight: '700' }]}>
                    Approve & Pay
                  </Text>
                </TouchableOpacity>
                <TouchableOpacity
                ...
                >
                  <Icon name="close-circle-outline" size={22} color={action === 'reject' ? '#F87171' : '#9CA3AF'} />
                  <Text style={[styles.actionBtnText, action === 'reject' && { color: '#F87171', fontWeight: '700' }]}>
                    Reject
                  </Text>
                </TouchableOpacity>
              </View>
              ...
              {action === 'approve' && (
                <>
                  ...
                  <View style={styles.amountInputWrap}>
                    ...
                    />
                    {parseFloat(editedAmount) !== advance.requestedAmount && parseFloat(editedAmount) > 0 && (
                  ...
                  <View style={styles.methodRow}>
                    {PAYMENT_METHODS.map(method => (
                      <TouchableOpacity
                        ...
                        style={[
                          styles.methodBtn,
                          paymentMethod?.id === method.id && { borderColor: method.color, backgroundColor: method.color + '15' },
                        ]}
                        onPress={() => setPaymentMethod(method)}
                      >
                        <Icon name={method.icon} size={22} color={paymentMethod?.id === method.id ? method.color : '#9CA3AF'} />
                        <Text style={[styles.methodLabel, paymentMethod?.id === method.id && { color: method.color, fontWeight: '700' }]}>
                          {method.label}
                        </Text>
                      </TouchableOpacity>
                    ))}
                  ...
                  <Text style={styles.fieldLabel}>Note to Employee (Optional)</Text>
                  <TextInput
                    style={[styles.textInput, { minHeight: normalise(60), textAlignVertical: 'top' }]}
                    value={approverNote}
                  ...
                  <View style={styles.summaryBanner}>
                    <View style={styles.summaryRow}>
                      ...
                      <Text style={styles.summaryValue}>₹{parseFloat(editedAmount || 0).toLocaleString('en-IN', { maximumFractionDigits: 2 })}</Text>
                    </View>
                    <View style={styles.summaryRow}>
                      <Text style={styles.summaryLabel2}>Via</Text>
                      <Text style={styles.summaryValue}>{paymentMethod?.label || '—'}</Text>
                    </View>
                    <View style={styles.summaryRow}>
                      <Text style={styles.summaryLabel2}>To</Text>
                      <Text style={styles.summaryValue}>{advance.employeeName}</Text>
                    </View>
                  </View>
                </>
              )}
              ...
              {action === 'reject' && (
                <>
                  ...
                  <TextInput
                    style={[styles.textInput, { minHeight: normalise(80), textAlignVertical: 'top' }]}
              ...
              <View style={styles.modalBtnRow}>
                ...
                <TouchableOpacity
                  style={[
                    styles.confirmBtn,
                    action === 'approve' ? styles.confirmBtnApprove :
                    action === 'reject'  ? styles.confirmBtnReject  :
      ...
      <View style={styles.card}>
        <View style={styles.cardTopRow}>
          ...
          <View style={styles.cardTopMiddle}>
            ...
            <Text style={styles.employeeEmail} numberOfLines={1}>{item.employeeEmail}</Text>
          </View>
        ...
        <TouchableOpacity style={styles.actionCardBtn} onPress={() => onAction(item)}>
...
export default function PendingAdvancesScreen(props) {
  ...
  const handleAction = advance => { setSelectedAdvance(advance); setShowModal(true); };

  const processAction = async ({ action, editedAmount, paymentMethod, reference, approverNote, rejectionReason }) => {
    ...
    try {
      ...
      await notifyEmployee(selectedAdvance, action, editedAmount, paymentMethod, rejectionReason);
      ...
      Alert.alert(
        ...
        action === 'approve'
          ? `₹${parseFloat(editedAmount).toLocaleString('en-IN', { maximumFractionDigits: 2 })} paid to ${selectedAdvance.employeeName} via ${paymentMethod.label}.`
  ...
  const notifyEmployee = async (advance, action, amount, paymentMethod, reason) => {
    try {
      ...
      await firestore().collection('notifications').add({
        ...
        body:      isApproved
          ? `₹${parseFloat(amount).toLocaleString('en-IN')} paid via ${paymentMethod.label} for your advance request.`
  ...
  const onRefresh = useCallback(() => { setRefreshing(true); fetchPendingAdvances(); }, []);
    ...
    <SafeAreaView style={styles.container}>
      ...
      <Animated.View style={[styles.header, { transform: [{ translateY: headerAnim }], opacity: headerOpacity }]}>
        ...
        <View>
          ...
          <Text style={styles.headerSubtitle}>{advances.length} request{advances.length !== 1 ? 's' : ''} waiting</Text>
        </View>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          renderItem={({ item, index }) => <AdvanceCard item={item} index={index} onAction={handleAction} />}
          ListEmptyComponent={
            <View style={styles.emptyContainer}>
              <Icon name="check-all" size={64} color="#E5E7EB" />
              <Text style={styles.emptyTitle}>All Clear!</Text>
              <Text style={styles.emptySubtitle}>No pending advance requests</Text>
            </View>
          }
...
const styles = StyleSheet.create({
  ...
  modalEmployeeRow: { flexDirection: 'row', alignItems: 'center', gap: 12, marginBottom: normalise(16) },
  ...
  modalEmployeeName: { fontSize: normalise(16), fontWeight: '700', color: '#1F2937' },
  modalEmployeeEmail: { fontSize: normalise(12), color: '#9CA3AF' },
  requestedBanner: { backgroundColor: '#1F2937', borderRadius: 14, padding: normalise(16), alignItems: 'center', marginBottom: normalise(16) },
  requestedLabel: { color: 'rgba(255,255,255,0.6)', fontSize: normalise(11), fontWeight: '600', textTransform: 'uppercase', letterSpacing: 0.5 },
  requestedAmount: { color: '#fff', fontSize: normalise(28), fontWeight: '800', marginVertical: normalise(4) },
  ...
  actionBtn: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, paddingVertical: normalise(14), borderRadius: 14, borderWidth: 1.5, borderColor: '#E5E7EB', backgroundColor: '#F9FAFB' },
  actionBtnApprove: { borderColor: '#34D399', backgroundColor: '#F0FFF8' },
  actionBtnReject:  { borderColor: '#F87171', backgroundColor: '#FFF5F5' },
  actionBtnText: { fontSize: normalise(13), color: '#9CA3AF', fontWeight: '600' },
  ...
  rupeeSign: { fontSize: normalise(20), color: '#34D399', fontWeight: '700', marginRight: 6 },
  amountInput: { flex: 1, fontSize: normalise(24), fontWeight: '800', color: '#1F2937' },
  editedBadge: { backgroundColor: '#FFFBEB', paddingHorizontal: normalise(8), paddingVertical: normalise(3), borderRadius: 20, borderWidth: 1, borderColor: '#F59E0B' },
  editedBadgeText: { color: '#F59E0B', fontSize: normalise(11), fontWeight: '700' },
  ...
  summaryLabel2: { fontSize: normalise(13), color: '#9CA3AF', fontWeight: '600' },
  summaryValue: { fontSize: normalise(13), color: '#1F2937', fontWeight: '700' },
  ...
  confirmBtn: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, paddingVertical: normalise(14), borderRadius: 12 },
