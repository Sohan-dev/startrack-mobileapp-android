  ...
  <Modal visible={visible} transparent animationType="slide">
    ...
    <View style={styles.sheet}>
      ...
      {APPROVERS.map(approver => (
        <TouchableOpacity
          key={approver.email}
          style={[styles.approverRow, selected?.email === approver.email && styles.approverRowActive]}
          onPress={() => { onSelect(approver); onClose(); }}
        >
          ...
          {selected?.email === approver.email && <Icon name="check-circle" size={20} color="#E8453C" />}
        </TouchableOpacity>
      ))}
    </View>
  </Modal>
...
export default function EditExpenseScreen(props) {
  ...
  const [entries, setEntries]     = useState(
    expense?.entries?.length
      ? expense.entries.map((e, i) => ({ ...e, id: i + 1, description: e.description || '' }))
      : [{ id: 1, type: null, amount: '', description: '' }],
  );
  const [approver, setApprover]               = useState(expense?.approver || '');
  const [approverEmail, setApproverEmail]     = useState(expense?.approverEmail || '');
  ...
  const addEntry = () => {
    setEntries(prev => [...prev, { id: nextId.current++, type: null, amount: '', description: '' }]);
  };
  ...
  const handleUpdate = async () => {
    ...
    if (!entries.length){ showErrorAlert('Please enter at least one entry');       return; }
    if (!approver)      { showErrorAlert('Please select approver');                return; }

    Alert.alert('Update Expense', 'Are you sure you want to update this expense?', [
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>{isReadOnly ? 'View Expense' : 'Edit Expense'}</Text>
        <View style={{ width: 38 }} />
      </View>
      ...
      {isReadOnly && (
        <View style={[
          ...
          />
          <Text style={[styles.statusBannerText, { color: expense?.status === 'Approved' ? '#34D399' : '#F87171' }]}>
      ...
      >
        ...
        {showDate && (
          <DateTimePicker
            ...
            mode="date"
            onChange={(e, selected) => { setShowDate(false); if (selected) setDate(selected); }}
          />
        )}
        ...
        <View style={styles.entriesCard}>
          ...
          {entries.map((entry, idx) =>
            isReadOnly ? (
              // ✅ Read-only view with description
              <View key={entry.id || idx} style={styles.readOnlyWrapper}>
                ...
                {/* ✅ Show description in read-only mode */}
                {entry.description ? (
                  <Text style={styles.readOnlyDesc}>📝 {entry.description}</Text>
                ) : null}
              </View>
            ) : (
              <EntryRow
...
const styles = StyleSheet.create({
  ...
  headerTitle: { color: '#fff', fontSize: normalise(18), fontWeight: '700', letterSpacing: 0.3 },
  ...
  statusBanner: { flexDirection: 'row', alignItems: 'center', gap: 8, marginHorizontal: normalise(16), marginTop: normalise(12), padding: normalise(12), borderRadius: 12, borderWidth: 1.5 },
  statusBannerText: { fontSize: normalise(13), fontWeight: '600', flex: 1 },

  sectionLabel: { fontSize: normalise(12), fontWeight: '700', color: '#6B7280', letterSpacing: 0.8, textTransform: 'uppercase', marginTop: normalise(18), marginBottom: normalise(8), marginLeft: normalise(4) },
  sectionHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: normalise(18), marginBottom: normalise(8) },

  card: { backgroundColor: '#fff', borderRadius: 14, padding: normalise(14), flexDirection: 'row', alignItems: 'center', elevation: 2, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.05, shadowRadius: 4 },
  dateIconWrap: { width: 44, height: 44, borderRadius: 12, backgroundColor: '#FFF0F0', justifyContent: 'center', alignItems: 'center', marginRight: normalise(12) },
  dateTextWrap: { flex: 1 },
  dateLabelSmall: { fontSize: normalise(10), color: '#9CA3AF', fontWeight: '600', textTransform: 'uppercase', letterSpacing: 0.4, marginBottom: 2 },
  dateValue: { fontSize: normalise(15), fontWeight: '700', color: '#1F2937' },

  addEntryBtn: { flexDirection: 'row', alignItems: 'center', gap: 5, backgroundColor: '#E8453C', borderRadius: 10, paddingHorizontal: normalise(12), paddingVertical: normalise(7) },
  addEntryBtnDisabled: { backgroundColor: '#FCA5A5' },
  addEntryText: { color: '#fff', fontSize: normalise(12), fontWeight: '700' },

  entriesCard: { backgroundColor: '#fff', borderRadius: 14, padding: normalise(12), elevation: 2, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.05, shadowRadius: 4 },
  entryHeader: { flexDirection: 'row', alignItems: 'center', paddingBottom: normalise(8), borderBottomWidth: 1, borderBottomColor: '#F3F4F6', marginBottom: normalise(8), gap: 8 },
  entryHeaderText: { fontSize: normalise(10), fontWeight: '700', color: '#9CA3AF', textTransform: 'uppercase', letterSpacing: 0.4 },

  // ✅ Entry wrapper + description
  entryWrapper: { marginBottom: normalise(10) },
  entryRow: { flexDirection: 'row', alignItems: 'center', marginBottom: 0, gap: 8 },
  entryDescInput: {
    marginLeft: normalise(32),
  ...
  entryIndex: { width: 24, height: 24, borderRadius: 12, backgroundColor: '#F3F4F6', justifyContent: 'center', alignItems: 'center' },
  entryIndexText: { fontSize: normalise(11), fontWeight: '700', color: '#6B7280' },
  entryTypeBtn: { flex: 1.2, flexDirection: 'row', alignItems: 'center', gap: 4, backgroundColor: '#F9FAFB', borderRadius: 10, paddingHorizontal: normalise(8), paddingVertical: normalise(10), borderWidth: 1.5, borderColor: '#E5E7EB' },
  entryTypeText: { fontSize: normalise(12), fontWeight: '700', flex: 1 },
  entryTypePlaceholder: { fontSize: normalise(12), color: '#C4C4C4', flex: 1 },
  entryAmountWrap: { flex: 1.5, flexDirection: 'row', alignItems: 'center', backgroundColor: '#F9FAFB', borderRadius: 10, borderWidth: 1.5, borderColor: '#E5E7EB', paddingHorizontal: normalise(8) },
  rupeeSign: { fontSize: normalise(14), color: '#6B7280', fontWeight: '700', marginRight: 4 },
  entryAmountInput: { flex: 1, fontSize: normalise(14), color: '#1F2937', fontWeight: '600', paddingVertical: normalise(10) },
  removeBtn: { padding: 4 },

  // ✅ Read-only with description
  readOnlyWrapper: { marginBottom: normalise(8) },
  readOnlyEntry: { flexDirection: 'row', alignItems: 'center', gap: 8 },
  readOnlyType: { flex: 1.2, flexDirection: 'row', alignItems: 'center', gap: 6, borderRadius: 10, paddingHorizontal: normalise(10), paddingVertical: normalise(8) },
  readOnlyTypeText: { fontSize: normalise(12), fontWeight: '700' },
  readOnlyAmount: { flex: 1.5, fontSize: normalise(14), fontWeight: '700', color: '#1F2937', textAlign: 'right' },
  readOnlyDesc: { fontSize: normalise(11), color: '#9CA3AF', fontStyle: 'italic', marginLeft: normalise(32), marginTop: normalise(3) },

  descInput: { flex: 1, fontSize: normalise(14), color: '#1F2937', paddingTop: 0, minHeight: normalise(60), textAlignVertical: 'top' },

  totalBanner: { backgroundColor: '#1F2937', borderRadius: 14, paddingVertical: normalise(16), paddingHorizontal: normalise(18), flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: normalise(20), marginBottom: normalise(14) },
  ...
  submitBtn: { backgroundColor: '#E8453C', borderRadius: 14, paddingVertical: normalise(16), flexDirection: 'row', justifyContent: 'center', alignItems: 'center', gap: 8, elevation: 4, shadowColor: '#E8453C', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 8 },
  submitText: { color: '#fff', fontSize: normalise(16), fontWeight: '700', letterSpacing: 0.3 },

  overlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.4)' },
  sheet: { backgroundColor: '#fff', borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: normalise(20), paddingBottom: normalise(36) },
  ...
  typeChip: { flexDirection: 'row', alignItems: 'center', gap: 8, backgroundColor: '#F9FAFB', borderRadius: 12, paddingHorizontal: normalise(12), paddingVertical: normalise(10), borderWidth: 1.5, borderColor: '#E5E7EB', minWidth: '44%' },
  ...
  approverRow: { flexDirection: 'row', alignItems: 'center', gap: 12, paddingVertical: normalise(12), borderBottomWidth: 1, borderBottomColor: '#F3F4F6' },
  approverRowActive: { backgroundColor: '#FFF5F5', borderRadius: 12, paddingHorizontal: normalise(8), borderBottomWidth: 0, marginBottom: 4 },
  approverAvatar: { width: 40, height: 40, borderRadius: 20, backgroundColor: '#E5E7EB', justifyContent: 'center', alignItems: 'center' },
